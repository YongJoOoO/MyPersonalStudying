...
class Solution{
	...
	static int N, K;
	static int[] arr;
	static ArrayList<Integer> team;//여기에 담기
	static int max;
	
	//DFS
	static void solution(int st) {
	...
	public static void main(String args[]) throws Exception {
		...
		*/
		...
		for (int test_case = 1; test_case <= T; test_case++) {
			...
			//2) DFS 각 i의 시작점으로 호출
			for(int i=0; i<N; i++) {
				team = new ArrayList<>(); //초기화
				solution(arr[i]);
				max = Math.max(max, team.size()); //가장 큰 팀 사이즈 
			}
			//3) 정답 세팅 
